...
#define BROWSER_DEFAULT_HOME "http://forums.winamp.com/index.php"
...
System.onScriptLoaded ()
{
	...
	navurl.setText(getPrivateString(getSkinName(), "Browser Home", BROWSER_DEFAULT_HOME));
...
bhome.onRightButtonUp(int x, int y)
{
	...
	if (ret == 1) setPrivateString(getSkinName(), "Browser Home", navurl.getText());
	if (ret == 2) setPrivateString(getSkinName(), "Browser Home", BROWSER_DEFAULT_HOME);
...
performWebSearch (string searchstring)
{
	...
	if (browser_search_attrib.getData() == "Plug-ins") 
		searchstring = "https://winampheritage.com/plugins/" + prepareWebString(searchstring, "");

	else if (browser_search_attrib.getData() == "Modern Skins")
		searchstring = "https://winampheritage.com/skins/" + prepareWebString(searchstring, "");
    else if (browser_search_attrib.getData() == "Classic Skins")
        searchstring = "https://skins.webamp.org/?query=" + prepareWebString(searchstring, "+");

	else if (browser_search_attrib.getData() == "Web Search") 
		//searchstring = "http://www.google.com/search/web/?q=" + prepareWebString(searchstring, "+"); - Rel@m: I won't use Google because Google killed support for IE11: https://9to5google.com/2021/10/01/google-search-internet-explorer-11/
		searchstring = "http://www.bing.com/search/web/?q=" + prepareWebString(searchstring, "+");
	...
	else if (browser_search_attrib.getData() == "Bands in Town") 
...
browser_search_attrib.onDataChanged ()
{
	...
	if (getData() == "Bands in Town")
    {
        bsearch.setXmlParam("image", "browser.button.search.bit.normal");
    }
...
createMenufromAttribList (list entries)
{
	...
	{
		if (dummy == 1) _menu.addSubMenu(submenus.enumItem(i), "Winamp Plugins/Skins"); //TODO ci.getName();
		else
		{
...
loadInternalLinks ()
{
	...
	storeInternalLink ("WACUP", "https://getwacup.com");
	storeInternalLink ("WinampHeritage", "https://winampheritage.com/");
	storeInternalLink ("1001skins", "http://www.1001skins.net/");
	storeInternalLink ("WinampPlugins.co.uk", "https://winampplugins.co.uk/");
	storeInternalLink ("Bing", "https://bing.com");

	// Internal Quick links - help & support
	storeInternalLink ("[[submenu]1]Winamp Help", "http://forums.winamp.com/forumdisplay.php?f=132");
	storeInternalLink ("[[submenu]1]Winamp Forums", "http://forums.winamp.com/");

	// Internal Quick links - MP3 Music blogs
	/* Rel@m: This will be disabled 'till the site is fixed (29/3/22) 
...
showQLEdit (int item)
{
	editItemNum = item;
	ql_edit_visible = 1;
	quicklink_name = newDynamicContainer("browser.quicklink.edit.dialog");
...
showSMEdit (int item)
{
	...
	smEditItemNum = item;
	sm_edit_visible = 1;
	submenu_name = newDynamicContainer("browser.submenu.edit");
